...
StereoForm::StereoForm(QWidget *parent) :
    QWidget(parent),
...
void StereoForm::on_pushButton_refresh_clicked()
{
    ...
    this->updateImage();
    emit this->updateImages(this->leftmat,this->rightmat,this->dismat);
    mutex.unlock();
}
void StereoForm::updateImage(){
    Mat leftshow,rightshow;
    if(this->leftmat.size().width>0){
...
void StereoForm::setImage(Mat &left, Mat &right, Mat &dis){
    this->leftmat = left.clone();
    this->rightmat = right.clone();
    this->initialForm();
    this->checkForm();
    stereo.stereoMatch(leftmat,rightmat,dismat);
    this->updateImage();
    dis = this->dismat.clone();
}
